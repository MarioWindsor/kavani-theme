...
class Wishlist {
    ...
    formatMoney(cents, format) {
        // Ensure cents is a number (it comes from data-product-price as a string, but represents cents)
        if (typeof cents === 'string') {
            cents = parseInt(cents, 10);
        }

        // Option 1: Use Shopify's built-in money formatter if available
        // This is usually the best as it respects your Shopify admin currency settings.
        if (typeof Shopify !== 'undefined' && typeof Shopify.formatMoney === 'function') {
            // Use the format passed, or fall back to window.Shopify.moneyFormat,
            // or provide a default if window.Shopify.moneyFormat isn't ready.
            const moneyFormat = format || window.Shopify.moneyFormat || 'Rs. {{ amount }}/-';
            return Shopify.formatMoney(cents, moneyFormat);
        } else {
            // Option 2: Basic fallback using Intl.NumberFormat for precise INR formatting
            // This runs if Shopify.formatMoney is not found.
            const formatter = new Intl.NumberFormat('en-IN', { // 'en-IN' for English (India) locale
                style: 'currency',
                currency: 'INR', // Set currency to Indian Rupees
                minimumFractionDigits: 2, // Ensure two decimal places (for paise)
                maximumFractionDigits: 2
            });
            let formattedAmount = formatter.format(cents / 100); // Convert cents to rupees

            // Append "/-" if it's not already part of the formatted string (Intl.NumberFormat doesn't add it)
            // Note: Intl.NumberFormat for INR usually gives '₹', so this will add '/-'
